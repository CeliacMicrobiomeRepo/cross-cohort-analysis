# Differential Abundance Code (ASV)

# Runs differential abundance analysis using ANCOM-BC2 at the ASV level for 16S phyloseq objects
...
# ── 2)  Assign ASV numbers ────────────────────────────
# for downstream zero-variance removal (ASV1...)
# Number of taxa
n_taxa <- ntaxa(ps)
...
outdir <- "16S/differential_abundance/stool_active/ASV"
...
# ── 5)  Remove zero-variance ASVS ────────────────────────────
#Note some ASVs will have zero variance and will need to be removed before ANCOM c("ASV1","ASV2")
bad_asvs <- c("")
...
## ── 10) Assemble long table of per-study effect sizes ─────────────────────
...
## ── 11) Random-effects REML meta-analysis per taxon ───────────────────────
...
## ── 12) Write full & significant-only results ─────────────────────────────
...
## ── 13) Pooled-sample ANCOM-BC² (combined analysis)  ───────────────────────────────
...
##  ── 16) CSV with *all* significant taxa (meta + pooled) + taxonomy ──────────────────────────
