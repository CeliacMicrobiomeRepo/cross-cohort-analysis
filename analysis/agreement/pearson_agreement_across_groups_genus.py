"""
pearson_agreement_across_groups_genus.py

Pearson agreement across groups at genus level
...
# ── 2) Load genus-level CSVs ────────────────────────────────────────────
files = sorted(DATA_DIR.glob("sig_taxa_combined_genus_*.csv"))
if not files:
    raise FileNotFoundError("No files matching 'sig_taxa_combined_genus_*.csv' found.")

def group_name(p: Path) -> str:
    # Make short labels
    name = p.stem.replace("sig_taxa_combined_genus_", "").replace(" - Copy", "").strip("_")
    return name
...
# ── 3) Load each file as a Series (genus: Effect) ────────────────────────────
series_by_group = {}
for p in files:
    df = pd.read_csv(p)

    genus_col = get_col(df, "genus")
    lfc_col = get_col(df, "lfc_pooled")
    est_col = get_col(df, "estimate_meta")

    if genus_col is None:
        raise KeyError(f"'genus' column not found in {p.name}")
    ...
    s = pd.Series(effect.values, index=df[genus_col].astype(str)).dropna()
    s = s.groupby(level=0).mean()  # average duplicates
    series_by_group[group_name(p)] = s

if len(series_by_group) < 2:
    raise RuntimeError("Need at least two genus files to compute pairwise correlations.")
...
if not records:
    raise RuntimeError("No overlapping genus found between any pair of groups.")


# ── 5) Save correlation summary ─────────────────────────────────────────────
summary = pd.DataFrame.from_records(records).sort_values("r", ascending=False)
summary_path = DATA_DIR / "pairwise_agreement_genus_summary.csv"
...
plt.title("Pairwise Pearson agreement across groups (genus level)")
...
plot_path = DATA_DIR / "pairwise_agreement_genus_lines.png"
